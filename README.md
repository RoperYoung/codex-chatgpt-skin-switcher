...
> [!TIP]
> **自定义上传图片 / Use your own images**
>
> 从 Mac 选择或拖入自己的图片，可让 ChatGPT 与 Codex 共用一张背景，也可为两个场景分别设置。图片只在本地处理和保存，不会上传云端。
>
> Choose or drag in your own images from your Mac. Share one background across ChatGPT and Codex or configure each scene separately. Images are processed and stored locally, never uploaded to the cloud.
...
- **Use your own images:** Choose or drag in local images from your Mac, then share one background or configure ChatGPT and Codex separately; images are never uploaded to the cloud
